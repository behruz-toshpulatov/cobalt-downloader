...
const spawnManaged = (name, args) => {
  ...
  child.on('error', (error) => {
    console.error(`[cobalt:dev] failed to start ${name}:`, error);
    shutdown(1);
  });
...
(async () => {
  const spawned = [
    spawnManaged('backend', ['--filter', '@cobalt/backend', 'start:dev']),
    spawnManaged('frontend', ['--filter', '@cobalt/frontend', 'dev'])
  ];
  ...
  if (!shuttingDown) {
    if (firstExit.error) {
      ...
      if (typeof code === 'number' && code !== 0) {
        console.error(`[cobalt:dev] ${firstExit.name} exited with code ${code}`);
        shutdown(code);
      } else if (signal) {
        console.warn(`[cobalt:dev] ${firstExit.name} exited due to signal ${signal}`);
        shutdown(0, signal);
      } else {
        shutdown(0);
      }
